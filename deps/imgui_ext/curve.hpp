...
namespace ImGui
{
    ...
    inline int Curve(const char *label, const ImVec2& size, const int maxpoints, ImVec2 *points)
    {
        ...
        //MERGES points that are too close together in the x axis. The closenes by default is 1.0 / 128.0
        int kill = 0;
        do
        {
            if (kill)
        ...
        if (hovered)
        {
            ...
            if (g.IO.MouseDown[0])
            {
                ...
                int sel = -1;
                //How close does the click need to be tothe control point in order to select it
                if (p1d < 0.1) sel = left;
                if (p2d < 0.1) sel = left + 1;
        ...
        RenderTextClipped(ImVec2(bb.Min.x, bb.Min.y + style.FramePadding.y), bb.Max, str, NULL, NULL, ImVec2(0.5f,1.0f));
