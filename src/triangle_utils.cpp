...
void triangle_improving_edge_flip(const Eigen::MatrixXd &V,
  ...
  std::cout << "starting triangle_improving_edge_flip" << '\n';
  using namespace Eigen;
  using namespace std;
  //          e0                 e0
  //          /|\                / \
  ...
  //EMAP is Fx3 and maps each f edge to an edge in E
...
//    assert(area_sq > -1e-10);
  ...
  Eigen::MatrixXd flap_qual(E.rows(), 2);
  flap_qual.setConstant(-2000);// init -1,
  // iso-to-EFlap
  ...
  // std::cout << "edges quality" << '\n';
  // std::cout << "nr of Faces is " << F.rows << '\n';
  // std::cout << "E has size " << E.rows() << '\n';
  // std::cout << "EF has size " << EF.rows() << " " << EF.cols() << '\n';
  // std::cout << "flap_qual has size " << flap_qual.rows() << " " << flap_qual.cols() << '\n';
  // std::cout << "face_quality has size " << face_quality.size() << '\n';
  for (size_t e = 0; e < E.rows(); e++){
    for (auto f:{0, 1}){
        // std::cout << "edge_quality for e and f " << e << " " << f << '\n';
        // std::cout << "accesing EF gives you " << EF(e, f) << '\n';
        if (EF(e, f) != -1){
            // std::cout << "is different than -1 " << '\n';
            flap_qual(e, f) = face_quality[EF(e, f)];
        }
    }
  }
   // std::cout << "finished edges quality" << '\n';
  ...
  while (true) { // well, it can never be empty.
    ...
    if (marker < edge_stamp[edge_id]){
        // std::cout << "timestamp and manifold check, we will cotninue" << '\n';
        continue;
    }
    ...
    if (v0 < 0 || v1 < 0){
        // std::cout << "skip boundary v0 and v1 are " << v0 << " " << v1  << '\n';
        continue; // skip boundary.
    }

    // if (f0< 0 || f1< 0){
    //     //my own skip boundary
    //     // std::cout << "own skip boundary f0 an f1 are " << f0 << " " << f1 << '\n';
    //     continue;
    // }
    assert(f0 >= 0 && f1 >= 0);
    ...
    //     std::cout << "failed manifold check, skipping" << '\n';
    //     continue;
    // }
...
void triangle_improving_edge_flip2(const Eigen::MatrixXd &V,
  ...
  std::cout << "starting triangle_improving_edge_flip2" << '\n';
  using namespace Eigen;
  using namespace std;
  //          e0                 e0
  //          /|\                / \
  ...
  Eigen::VectorXd edges_quality;
  edges_quality=1/Elen.array(); // TODO should depend on the nr of points swept (or maybe the quality of those that are vertical should be compltetly dissalowed)


  auto qualities_for_edge = [&](size_t e) -> double {
      return edges_quality(e);
  };
  auto quality_after = [&](size_t e) -> double {
    if (EV(e, 1) < 0 || EV(e, 0) < 0) return -1;

    double dist = (V.row(EV(e, 0)) -   V.row(EV(e, 1))).norm();
    return 1/dist;
  };
  auto quality_improvement = [&](size_t e) -> double {
    double before = qualities_for_edge(e);
    double after = quality_after(e);
    return after - before;
  };
  ...
  while (true) { // well, it can never be empty.
    ...
    // perform flip: handle F, E, EMAP, EF, EV
    std::cout << "performing flip of edge " << edge_id  << " with improv " << q_improv << '\n';
    // auto& half_edges = uE2E[edge_id];
    // const size_t e_12 = half_edges[0];
    // std::cout << "E(e_12) and E(edge_id) " << E.row(e_12) << " " << E.row(edge_id) << '\n';
    ...
    igl::flip_edge(F, E_all, E, EMAP_vec, uE2E, edge_id);
    nr_of_flips_done++;
...
//     assert(EMAP(f1, m1) == edge_id);
//
//     std::cout << "changing from edge " << E.row(edge_id) << " to " << v1 << " " <<  v0  << '\n';
//     E.row(edge_id) << v1, v0; // to be compatible with igl::flip_edge
//     EV.row(edge_id) << e0, e1;
//     // EF.row(edge_id) << EF.row(edge_id);
//     // The three lines above determines the ordering below
...
//     }
    ...
    //SCAF update qualities
    // double new_edge_qual = 1/((V.row(E(edge_id, 0)) -   V.row(E(edge_id, 1))).norm());
    // std::cout << "new_edge is " << E.row(edge_id) << '\n';
    // std::cout << "new_edge_qual is " << new_edge_qual << '\n';
    // edges_quality(edge_id)= new_edge_qual;
    //
    // // improve
    // edge_heap.push_back(std::make_tuple(-q_improv, edge_id,
    //                                     ++edge_stamp[edge_id]));
    ...
    // // }
    //
    // //get the faces that contain the new edge we created
    // int _m0, _m1;
    // const size_t num_faces = F.rows();
    // const auto& adj_faces = uE2E[edge_id];
    // int f0=-1, f1=-1;
    // if (adj_faces.size() == 2) {
    //   f0  = adj_faces[0] % num_faces;
    //   f1 = adj_faces[1] % num_faces;
    // }
    // for (auto m:{0, 1, 2}) {
    //   if (EMAP(f0, m) == edge_id) _m0 = m;
    //   if (EMAP(f1, m) == edge_id) _m1 = m;
    ...
    //
    // //new edge qualities
    // double new_edge_qual = (V.row(E(edge_id, 0)) -   V.row(E(edge_id, 1))).norm();
    // edges_quality(edge_id)=new_edge_qual;
    //
    // // improve
    // edge_heap.push_back(std::make_tuple(-q_improv, edge_id,
...
//works kinda but doesn't seem to actually iterate through all of the edges (it's based on edge lenghts)
void triangle_improving_edge_flip3(const Eigen::MatrixXd &V,
  ...
  Eigen::VectorXd edges_quality;
  edges_quality=1.0/Elen.array(); // TODO should depend on the nr of points swept (or maybe the quality of those that are vertical should be compltetly dissalowed)
  ...
  auto quality_after = [&](size_t e) -> double {
    if (EV(e, 1) < 0 || EV(e, 0) < 0) return -1; // abuting vertex around the edge is not there so the edge is a border one

    // creating the new faces changes the normals way too much
      Eigen::MatrixXd F0(2, 3);
      F0 << E(e, 0), EV(e, 1), EV(e, 0),
              E(e, 1), EV(e, 0), EV(e, 1);
      Eigen::MatrixXd NF;
      igl::per_face_normals(V,F0, NF);
      double normal_deviation = NF.row(0).dot(NF.row(1));
      if(normal_deviation<0.7){
          // std::cout << "normal deviation is " << normal_deviation << "\n";
          return -1;
      }
  ...
  std::vector<improve> edge_heap(E.rows()); // init as 0,0,0s
    ...
    // std::cout << "creating vector of edge_heap" << "\n";
  for (size_t e = 0; e < E.rows(); e++) {
    edge_heap[e] = std::make_tuple(quality_improvement(e), e, 0);
  }
    // std::cout << "making the vector into a heap" << "\n";
  std::make_heap(edge_heap.begin(), edge_heap.end());
...
//    }
//
//    //becayse sometimes when we flip it it ends up in an edge with the same endpoints and cost is inf
//    if (v0==v1){
//        std::cout << "skipping because we v0 and v1 are the same " << v0 << " " << v1 << '\n';
//        std::cout << "f0 and f1 are  " << f0 << " " << f1 << '\n';
//        std::cout << "faces f0 and f1 are  " << F.row(f0) << " " << F.row(f1) << '\n';
//        continue;
//    }
//    assert(f0 >= 0 && f1 >= 0);
//
...
//    }
    ...
    assert(E_all(e_12, 0) == v1);
    assert(E_all(e_12, 1) == v2);
    assert(E_all(e_24, 0) == v2);
    assert(E_all(e_24, 1) == v4);
    assert(E_all(e_41, 0) == v4);
    assert(E_all(e_41, 1) == v1);
    assert(E_all(e_21, 0) == v2);
    assert(E_all(e_21, 1) == v1);
    assert(E_all(e_13, 0) == v1);
    assert(E_all(e_13, 1) == v3);
    assert(E_all(e_32, 0) == v3);
    assert(E_all(e_32, 1) == v2);

    const size_t ue_24 = EMAP_vec(e_24);
    const size_t ue_41 = EMAP_vec(e_41);
    const size_t ue_13 = EMAP_vec(e_13);
    const size_t ue_32 = EMAP_vec(e_32);
    ...
    EMAP_vec(new_e_34) = edge_id;
    EMAP_vec(new_e_43) = edge_id;
    EMAP_vec(new_e_41) = ue_41;
    EMAP_vec(new_e_13) = ue_13;
    EMAP_vec(new_e_32) = ue_32;
    EMAP_vec(new_e_24) = ue_24;

    auto replace = [](std::vector<int>& array, int old_v, int new_v) {
    std::replace(array.begin(), array.end(), old_v, new_v);
    };
    replace(uE2E[edge_id], e_12, new_e_34);
    replace(uE2E[edge_id], e_21, new_e_43);
...

    {
     ...
     // std::cout << "the edge is " << E.row(edge_id) << '\n';
     // std::cout << "f0 and f1 are " << F.row(f0) << " " << F.row(f1) << '\n';

     for (auto m:{0, 1, 2}) {
       if (EMAP(f0, m) == edge_id) _m0 = m;
       if (EMAP(f1, m) == edge_id) _m1 = m;
     }
     // std::cout << "_m0 and _m1 are " << _m0 << " " << _m1 << '\n';
     ...
     //new edge qualities
     double new_edge_qual = (V.row(E(edge_id, 0)) -   V.row(E(edge_id, 1))).norm();
     edges_quality(edge_id)=1/new_edge_qual;
     ...
     edge_heap.push_back(std::make_tuple(quality_improvement(edge_id), edge_id,
                                         ++edge_stamp[edge_id]));
     std::push_heap(edge_heap.begin(), edge_heap.end());
     for (auto e: {m02, m11, m12, m01}) {
       edge_heap.push_back(std::make_tuple(quality_improvement(e), e,
   ...
   // Eigen::VectorXd Elen_recomputed;
   // igl::edge_lengths(V, E, Elen_recomputed);
   // Eigen::VectorXd edge_quality_recomputed= 1.0/Elen_recomputed.array();
   // for (size_t i = 0; i < E.rows(); i++) {
   //     std::cout << "E lengh was " << edges_quality(i) << " now is " << edge_quality_recomputed(i)  << '\n';
   //
   //     if(  std::fabs(edges_quality(i)-edge_quality_recomputed(i)) < 0.0001 ){
   //         std::cout << "same" << '\n';
   //     }else{
...
void triangle_improving_edge_flip4(const Eigen::MatrixXd &V,
  ...
  auto min_qlty_after = [&](size_t e) -> double {
    ...
    //if it's a horizontal edge then we return -1 (horizontal edges have consecutive vertices)
    int diff= std::abs(E(e,0) - E(e,1));
    if(diff==1){
        return -1;
    }
